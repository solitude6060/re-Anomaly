...
class AnomalyCLIPHead(BaseHead):
    """
    AnomalyCLIP-style zero-shot head using CLIP text-image alignment.

    This implementation focuses on object-agnostic prompt alignment with
    optional global feature adaptation.
    """
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.object_name = config.get("object_name", "object")
        self.image_size = config.get("image_size", 224)
        self.temperature = config.get("temperature", 1.0)
        self.use_object_agnostic = config.get("use_object_agnostic", True)
        self.use_global_adapter = config.get("use_global_adapter", True)
        self.adapter_dim = config.get("adapter_dim", 256)
        ...
        self.agnostic_normal_prompts = config.get(
            "agnostic_normal_prompts",
            [
                "a photo of a normal object.",
                "a good photo of a normal object.",
                "a flawless object.",
                "a perfect object.",
            ],
        )
        self.agnostic_abnormal_prompts = config.get(
            "agnostic_abnormal_prompts",
            [
                "a photo of a damaged object.",
                "a photo of a defective object.",
                "an object with defects.",
                "an object with anomalies.",
            ],
        )

        self.visual_adapter = nn.Linear(self.embed_dim, self.embed_dim)
        self.global_adapter = nn.Sequential(
            nn.Linear(self.embed_dim, self.adapter_dim),
            nn.GELU(),
            nn.Linear(self.adapter_dim, self.embed_dim),
        )
    ...
    def _format_prompts(self, prompts: list[str]) -> list[str]:
        token = self.category if not self.use_object_agnostic else self.object_name
        return [p.format(token) for p in prompts]
    ...
    def _ensure_text_embeddings(self) -> None:
        ...
        if self._normal_embed is None:
            if self.use_object_agnostic:
                normal_texts = self.agnostic_normal_prompts
            else:
                normal_texts = self._format_prompts(self.normal_prompts)
            with torch.no_grad():
        ...
        if self._abnormal_embed is None:
            if self.use_object_agnostic:
                abnormal_texts = self.agnostic_abnormal_prompts
            else:
                abnormal_texts = self._format_prompts(self.abnormal_prompts)
            with torch.no_grad():
    ...
    def forward(self, features: list[torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        normal_embed: torch.Tensor = cast(torch.Tensor, self._normal_embed)
        abnormal_embed: torch.Tensor = cast(torch.Tensor, self._abnormal_embed)

        if self.use_global_adapter:
            global_feat = visual_feat.mean(dim=1)
            delta = self.global_adapter(global_feat)
            normal_embed = F.normalize(normal_embed + delta, dim=-1)
            abnormal_embed = F.normalize(abnormal_embed - delta, dim=-1)
