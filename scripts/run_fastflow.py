"""
FastFlow evaluation script for MVTec AD.

FastFlow uses 2D normalizing flows for anomaly detection.
Requires only OK/NG labels - no segmentation masks needed.
"""
...
def train_fastflow(
    ...
    """Train FastFlow head on normal samples."""

    # First, fit to initialize the flow
    print("  Initializing FastFlow...")
    backbone.eval()
    with torch.no_grad():
    ...
    # Setup optimizer - only train the flow and projection layers
    params = list(head.parameters())
    optimizer = optim.Adam(params, lr=lr)
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs)

    print(f"  Training FastFlow for {epochs} epochs...")
...
def evaluate_category(
    ...
    """Train and evaluate FastFlow on a single category."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run FastFlow evaluation on MVTec AD")
    parser.add_argument(
        "--data_root",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Training epochs per category",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
...
def main():
    ...
    summary = {
        "experiment": f"FastFlow - {backbone_name}",
